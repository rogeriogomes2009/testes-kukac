...
class MotoControle {
  store(request, response) {
    ...
    if (typeof ano !== 'number' || typeof passageiros !== 'number') {
      return response.status(400).json({ error: 'Favor preencher os campos ano e passageiros com números' });
    }


    if (![1, 2].includes(passageiros)) {
      return response.status(400).json({ error: 'Favor definir se 1 ou 2 passageiros' });
    }


    DATABASE.motos.push(new Moto(
      modelo,
      ano,
      marca,
      passageiros
